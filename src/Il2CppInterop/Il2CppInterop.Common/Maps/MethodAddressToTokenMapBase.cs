...
public abstract class MethodAddressToTokenMapBase<TAssembly, TMethod> : IDisposable, IEnumerable<(long, TMethod?)>
{
    ...
    public MethodAddressToTokenMapBase(string filePath)
    {
        ...
        if (myHeader.Magic != Magic)
        {
            ...
            throw new FileLoadException(
                $"File magic mismatched for {filePath}; Expected {Magic:X}, got {myHeader.Magic:X}");
        }
        ...
        if (myHeader.Version != Version)
        {
            myMapFile.Dispose();
            throw new FileLoadException(
                $"File version mismatched for {filePath}; Expected {Version}, got {myHeader.Version}");
        }

        var offset = Marshal.SizeOf<MethodAddressToTokenMapFileHeader>();
        using var reader = new BinaryReader(myMapFile.CreateViewStream(offset, 0, MemoryMappedFileAccess.Read),
            Encoding.UTF8, false);
    ...
    IEnumerator<(long, TMethod?)> IEnumerable<(long, TMethod?)>.GetEnumerator()
    {
        return GetEnumerator();
    }
    ...
    public class Enumerator : IEnumerator<(long, TMethod?)>
    {
        ...
        public unsafe bool MoveNext()
        {
            ...
            if (myOffset < myMap.myHeader.NumMethods)
            {
                ...
                Current = (myMap.myPointers[myOffset],
                    myMap.ResolveMethod(myMap.myAssemblyList[assemblyIdx], dataToken));
